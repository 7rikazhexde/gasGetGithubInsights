 ...
 */
function getGithubInsightsTrafficClones (repositry_name) {
  ...
  // Request to GitHub
  const requestUrl ='https://api.github.com/repos/' + username + '/' + repositry_name + '/traffic/clones';
  const response = UrlFetchApp.fetch (requestUrl, options);
  const jsonData = JSON.parse (response);
  const rows = jsonData.clones.length;
  ...
  for (var i = 0; i < rows; i++) {
    ...
    mySheet.getRange (2, 1, 1, 1).setValue (jsonData.clones[i].timestamp.substr(0, 10));
    mySheet.getRange (2, 2, 1, 1).setValue (jsonData.clones[i].count);
    mySheet.getRange (2, 3, 1, 1).setValue (jsonData.clones[i].uniques);
  }
...
function main(){
  ...
  for(i=0; i<repoList_length; i++){
    ...
    getGithubInsightsTrafficClones(repoList[i]);
  }
